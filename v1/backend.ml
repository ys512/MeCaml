...
let rec translate ((comp, t):Tst.tcomp) = 
  ...
  | Block (a, c), _       -> translate_block a c (size_type t)
...
and translate_if c1 c2 c3 =
  sprintf "(if %s %s %s)" 
  (translate c1) (translate c2) (translate c3)

and translate_pair (c1, t1) (c2, t2) = 
  let w1, b1 = Util.bits_to_words (size_type t1) in
  let w2, b2 = Util.bits_to_words (size_type t2) in
  sprintf "(apply (global $Mem $combine) %s %d %d %s %d %d)" 
  (translate (c1, t1)) w1 b1 (translate (c2, t2)) w2 b2 

and translate_block a c size = sprintf "(apply (global $Mem $resize) %s %d)" (translate_pair a c) size
...
let translate_seq seq = 
  let exported = mk_export seq in
  let debug = "(_ (apply (global $Mem $debug) $out))" in
  sprintf "(module
  %s%s
(export %s))"
  (String.concat "\n  " (List.map translate_compdef seq))
  (if List.mem "$out" exported then "\n" ^ debug else "")
