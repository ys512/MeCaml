...
let roundup n = ((n-1)/Sys.word_size+1) * Sys.word_size

let rec trace c s loc = 
  match s with
  | [] -> loc
  | bs::s -> (
    let new_loc = 
      match bs with
      | SInt n              -> sprintf "(+ %s %d)" loc n
      | SRef                -> sprintf "(apply $add_ref %s)" loc
      | SMatch (stag, cases)  -> 
        let aux_loc = new_aux () in
        bind aux_loc loc
        (sprintf "(switch (apply (global $Mem $extract %s %s %d)) %s)"
        c loc stag (String.concat "\n" (List.map 
          (fun (tag, size) -> sprintf "(%d %s)" tag (trace c (SInt stag::size) aux_loc)) cases))) in
    trace c s new_loc
  )

let translate_unit = "(block (tag 0))"

let translate_int n = string_of_int n

let translate_bool b = if b then "1" else "0"
...
and translate_bop op c1 c2 =
  ...
  (translate_operator op) (translate c1) (translate c2)
...
and translate_pair c1 c2 t1 t2 = 
  let c1 = translate c1 in
  let c2 = translate c2 in
  let aux_1 = new_aux () in
  let aux_2 = new_aux () in
  let s1 = trace aux_1 (Size.size t1) "0" in
  let s2 = trace aux_2 (Size.size t2) "0" in
  sprintf "(apply (global $Mem $combine) %s %s %s %s)" aux_1 s1 aux_2 s2 
